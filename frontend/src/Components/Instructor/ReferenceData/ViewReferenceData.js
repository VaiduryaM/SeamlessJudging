...
const ViewReferenceData = (props) => {
  ...
  const formOnFinish = async (values) => {
    ...
    var id = selectedRowsByType[params.type][0];
    if(values.sponsor_id){
      id = values.sponsor_id;
    }
    ...
    if (result.status === 200) {
      ...
      var modalClose = document.querySelectorAll(".ant-modal-confirm-info .ant-modal-close");
      ...
      case "sponsor":
        ...
        setColumns({
          ...
          sponsor: RefDataColumnDefinitions("sponsors", {
            ...localTableFilters,
            sponsors: tempFilter,
          }, null, "listPage", editForm, formOnFinish, sponsersFormCompoenent, onCloseEditModal).filter((k) => !k.hidden),
        });
        break;
      case "winner_categories":
  ...
  const [columns, setColumns] = useState({
    sponsor: RefDataColumnDefinitions("sponsors", localTableFilters, null, "listPage").filter(
      (k) => !k.hidden
    ),
